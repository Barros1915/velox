...
Uso:
    from pycore.websocket import WebSocketManager, ws_route
...
class WebSocketManager:
    ...
    async def connect(self, ws, room: str = 'default', 
        ...
        # Iniciar heartbeat se ainda não estiver
...
class WebSocketHandler:
    ...
    Uso:
        ...
        @app.websocket('/ws/chat')
        async def chat(ws):
            await handler.handle(ws, require_room=True)
    """
...
def ws_route(path: str, manager: WebSocketManager = None):
    ...
    def decorator(fn):
        async def wrapper(ws):
            mgr = ws_manager or WebSocketManager()
